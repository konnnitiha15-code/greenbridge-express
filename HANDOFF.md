...
### 🟡 言語切替の対応漏れ
applyL() で更新される ID は付与済み。**2026-06-02 にシフトラベルの未対応を解消**（下記セッション履歴 #57）。
まだ残る可能性のある箇所（必要時に対応）：
- ホーム画面の動的なデータ系テキスト（「8日」「1年目」などの単位）
- モーダルラベルの一部
...
48. **有給管理（Phase5・自動付与+手動調整 / ワーカー申請→管理者承認 / 残数管理+申請承認）**
    ...
    - 管理者UI: 新規ページ `leave`(サイドバー nav-leave・承認待ちバッジ)。申請承認リスト + 残数一覧 + 一括同期 + 明細/手動調整モーダル
...
49. **健康診断・資格管理（Phase6）**：健診/フルハーネス/玉掛け/フォークリフト/技能講習等の有効期限を一元管理
    - マイグレーション **016**（worker_certifications テーブル + RLS + updated_atトリガ）。**2026-06-01 適用済み✅**（未実行時は資格UIが「準備中」503フォールバック）
...
### 2026-06-02 セッション（マイグレーション017適用 — Phase7完全有効化）
...
57. **多言語UI整理（ワーカーのシフトラベル）**：言語切替に追従しない日本語固定箇所を解消。
